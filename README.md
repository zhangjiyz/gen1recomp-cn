...
本项目使用 LÖVE2D 和 Lua 重新实现《宝可梦 红／蓝／黄》的运行逻辑，并正在逐步支持《宝可梦 金／银／水晶》。它不是传统 Game Boy 模拟器：程序会读取玩家提供的正版 ROM，验证版本并提取运行所需的数据，之后由项目自身实现的 Lua 引擎运行游戏。

> 本仓库不包含、不提供也不会下载任何 ROM。请使用自己合法取得的游戏 ROM。

## 中文版做了什么

这个分支以尽量减少上游代码改动、方便后续同步官方 `dev` 分支为原则，主要进行应用界面的中文本地化：
...
本分支以官方仓库的 `dev` 分支为上游基础。中文改动尽量集中在本地化目录和少量入口代码中，以降低同步上游更新时的冲突。
